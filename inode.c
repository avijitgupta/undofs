			 ...
			 struct inode *new_dir, struct dentry *new_dentry)
{
	...
	if(lower_new_dentry->d_op)
	{
		...
		{
			lower_new_dentry->d_op->d_revalidate(lower_new_dentry,
									 &nd);
		}
	}
...
static int wrapfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	printk(KERN_INFO "Initial error %d", err);
	#endif
	...
	err_dentry = wrapfs_lookup(trashbin_dentry->d_inode,
						 user_trashbin_dentry, &nd);
	...
	#ifdef DEBUG
	printk(KERN_INFO "user_trashbin error %d", err);
	#endif
	...
	if(user_trashbin_dentry->d_inode == NULL) {
		...
		err = wrapfs_mkdir(trashbin_dentry->d_inode,
	...
	#ifdef DEBUG
	printk(KERN_INFO "user tarshbin error %d", err);
	#endif
	...
	while(path_original[pos]!=0) {
		if(path_original[pos] == '/') {
			...
			orig_temp_dentry =  d_alloc(orig_parent_dentry,
								&temp_qstr);
			...
			if(IS_ERR(err_dentry)){
				#ifdef DEBUG
				printk(KERN_INFO "Loop: lookup-II %d", err);
				#endif
			...
			if(path_original[pos]!=0) {
				...
				err = wrapfs_mkdir(parent_dentry->d_inode,
					...
					{
					/*
					 * Overriding the permission settings
					 * done by umask(2)
					 */
					temp_dentry->d_inode->i_mode =
								temp_imode;
				...
				/* update trashbin parent & original parent */
				dput(parent_dentry);
	...
	if(renamed_dentry->d_inode) {
		...
		if(strlen(temp_name) + strlen(timestamp_string) > PAGE_SIZE) {
			...
			#ifdef DEBUG
			printk(KERN_INFO "-EEXIST, length > PAGE size");
			#endif
		...
		#ifdef DEBUG
		printk(KERN_INFO "hash error %d", err);
		#endif
		...
		if(IS_ERR(err_dentry) && err!=-ENOENT){
			#ifdef DEBUG
			printk(KERN_INFO "lookup: err trash_dentry %d", err);
			#endif
		...
		if(trash_dentry->d_inode) {
			#ifdef DEBUG
			printk(KERN_INFO "-EEXIST: Renamed hash file exists");
			#endif
		...
		err = wrapfs_rename(dir, dentry, parent_dentry->d_inode,
								trash_dentry);
		if(err < 0){
			#ifdef DEBUG
		...
		err = vfs_path_lookup(parent_dentry, current->fs->pwd.mnt,
						temp_name,0, &final_path);
		if(err < 0){
			#ifdef DEBUG
...
/* dput and d_drop dentries, release buffers */
	#ifdef DEBUG
	printk(KERN_INFO "Dentry Dropped %d", err);
	#endif
...
int restore(char* file_name, struct super_block* sb)
{
	...
	struct dentry* original_dentry = NULL ;
	...
	while(original_path[pos]!=0) {
		if(original_path[pos] == '/') {
			...
			err_dentry = wrapfs_lookup(
				trashbin_parent_dentry->d_inode,
			...
			/* Overriding the permission settings by umask(2)*/
				...
				trashbin_parent_dentry =
						dget(trashbin_temp_dentry);
			}
			else {
				restore_dentry = dget(temp_dentry);
				path_terminal_dentry = dget(
							trashbin_temp_dentry);
			}
			/* dput, d_drop these for reassigning in the loop */
			dput(temp_dentry);
			d_drop(temp_dentry);
			dput(trashbin_temp_dentry);
			d_drop(trashbin_temp_dentry);
	...
	if(restore_dentry->d_inode) {
		 ...
		 * access time of the file to be restored */
			...
			nd.flags = file_type? 0: LOOKUP_DIRECTORY;
			err_dentry = wrapfs_lookup(
			trashbin_parent_dentry->d_inode, trash_dentry, &nd);
			...
			err = wrapfs_rename(parent_dentry->d_inode,
				restore_dentry,trashbin_parent_dentry->d_inode,
							trash_dentry);
			...
			err = wrapfs_rename(trashbin_parent_dentry->d_inode,
					path_terminal_dentry,
					new_restore_dentry->d_parent->d_inode,
					new_restore_dentry);
			if(err < 0) {
				goto flag_put;
			}
			...
			err = vfs_path_lookup(trashbin_parent_dentry,
				current->fs->pwd.mnt , temp_name, nd.flags ,
							 &trash_temp_path);
			if(err !=0)
				goto dentry_put;
			...
			/*
			 * Instantiate a new negative dentry of the original
			 * name to rename at
			 */
			fetch_qstr(&original_qstr, new_restore_name);
			original_dentry = d_alloc(trashbin_parent_dentry,
							&original_qstr);
			err_dentry = wrapfs_lookup(
			trashbin_parent_dentry->d_inode, original_dentry, &nd);
			err = PTR_ERR(err_dentry);
			if(IS_ERR(err_dentry) && err!=-ENOENT){
	 ...
	 * wrapfs_rename to move the file/dir */
		...
		err = wrapfs_rename(trashbin_parent_dentry->d_inode,
		path_terminal_dentry, parent_dentry->d_inode, restore_dentry);
		if(err < 0){
			goto flag_put;
		}
		...
		nd.flags = file_type? 0: LOOKUP_DIRECTORY;
		err = vfs_path_lookup(parent_dentry,current->fs->pwd.mnt,
			restore_dentry->d_iname, nd.flags, &final_path);
		if(err < 0)
			goto flag_put;
		path_put(&final_path);
	}
...
static int wrapfs_normal_rmdir(struct inode *dir, struct dentry *dentry)
{
        ...
        if (err)
                goto out_unlock;
        rr = vfs_rmdir(lower_dir_dentry->d_inode, lower_dentry);
        if (err)
                goto out;
...
static int wrapfs_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	temp_name[i-1] = 0; // terminating null .
	...
	if(strcmp(path_original, "/.trash/") == 0 ||
			strcmp(path_original, "/.trash//")==0 ||
				strcmp(path_original, "/.trash") == 0) {
		#ifdef DEBUG
		printk(KERN_INFO
			"Deleting global trash: Operation not permitted");
		#endif
		err = -EACCES;
		goto free_path_original;
	}
	...
	#ifdef DEBUG
	printk(KERN_INFO "Path 2 %s Temp Name %s", path_original, temp_name);
	#endif
	...
	orig_parent_dentry = dget(sb->s_root);
	#ifdef DEBUG
	printk(KERN_INFO "Path is: %s %c", path_original, path_original[pos]);
	#endif
	len_name = 0;
	...
	while(path_original[pos]!=0) {
		if(path_original[pos] == '/') {
			...
			/* Overriding the permission settings by umask(2)*/
				temp_dentry->d_inode->i_mode = temp_imode;
				lower_inode =
				wrapfs_lower_inode(temp_dentry->d_inode);
				lower_inode->i_mode = temp_imode;
				}
...
out_end:
	#ifdef DEBUG
	printk(KERN_INFO "rmdir return value %d", err);
	#endif
	return err;
}
